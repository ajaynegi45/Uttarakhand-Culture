        ...
        <div className="min-h-screen bg-background">
            ...
            <section className="pt-32 pb-12 px-2 sm:px-6 bg-background border-b border-border">
                <div className="max-w-7xl mx-auto">
                    ...
                    >Explore the Heritage</motion.h1>
                            ...
                            return (
                                <motion.button
                                    ...
                                    className={cn(
                                        ...
                                        cat.colorCss,
                                        isSelected ? "ring-2 ring-offset-1 ring-ring scale-105" : ""
                                    )}>
                ...
                {/*<div className="max-w-7xl mx-auto px-4 sm:px-6">*/}
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div className="text-sm text-muted-foreground" aria-live="polite">
                            {isFiltering ? ( <span>Found <strong>{filteredFeatures.length}</strong> result{filteredFeatures.length !== 1 ? "s" : ""} </span> ) : ( <span>Showing all {filteredFeatures.length} items</span> )}
                        </div>
                    </div>
                ...
                <div role="list" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    { filteredFeatures.length === 0 ? (
                        <div role="listitem" className="col-span-full p-8 rounded-2xl bg-muted/20 text-center">
                            ...
                            <p className="text-muted-foreground">Try broader keywords, different spellings, or clear the category filter.</p>
                            <div className="mt-6">
                                <button
                                    onClick={() => {
                                        setSearchQuery("");
                     ...
                     filteredFeatures.map((item, i) => (
                         <article role="listitem" key={item.id} className="group cursor-pointer h-full flex flex-col">

                         <Link key={item.title} href={item.url} className="no-underline" prefetch={false}>
                            <motion.div
                                initial={{ opacity: 0, y: 20 }}
